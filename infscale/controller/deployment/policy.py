...
class DeploymentPolicy(ABC):
    ...
    def _get_device(
        ...
        """Return device based on available resources."""
        # decide the other device type
        # we might keep this here so we can use it
        # if we decide to add a preferred device type
        ...
        # get device based on dev_type
        device = self._get_available_device(preferred_dev, dev_type)

        if device is None:
            # get device based on other_dev_type
    ...
    def get_workers(
        ...
        curr_worker_ids = [
            wid for workers in distribution.values() for wid, _ in workers
        ]
    ...
    def _has_resources(
        ...
        assigned_workers = (
            0 if agent_id not in distribution else len(distribution[agent_id])
        )
