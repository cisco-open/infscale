...
class AgentContext:
    ...
    def keep_alive(self):
        ...
        self.logger.debug(f"keeping agent context alive for {self.id}")
        self.alive = True

        if not self.timer:
            self.timer = Timer(
                DEFAULT_TIMEOUT, self.ctrl.reset_agent_context, self.id
            )
        self.timer.renew()

    def reset(self):
        """Reset the agent context state."""
        self.logger.debug(f"agent {self.id} context reset")
        self.alive = False
        self.timer = None
