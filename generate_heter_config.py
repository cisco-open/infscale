...
def generate_unified_allocation(
    pipeline_stages, gpus_per_machine, cluster_ips, machine_offset=0, stage_id_offset=0
):
    """
    ...
    for stage in pipeline_stages:
        for machine_id_str, count in stage["gpu_allocation"].items():
            ...
            assert machine_id < len(
                cluster_ips
            ), f"Machine ID {machine_id} is out of range for the number of machines ({len(cluster_ips)})"
            machine_worker_count[machine_id] += count

            if machine_worker_count[machine_id] > gpus_per_machine:
                raise ValueError(
                    f"Machine {machine_id} has {machine_worker_count[machine_id]} workers allocated, but only {gpus_per_machine} GPUs per machine are configured."
                )
            ...
            for _ in range(count):
                if worker_idx < stage["num_replicas"]:
                    ...
                    for local_gpu in range(gpus_per_machine):
                        ...
                        raise ValueError(
                            f"No available GPUs on machine {machine_id} for worker {wid}"
                        )
...
def load_ip_config(ip_yaml_path: str):
    """Load dispatcher and per-GPU-class IPs from YAML.

    Expected format:
    dispatcher_ip: "10.20.1.100"  # Dedicated machine for dispatcher (must be GPU-enabled)
    gpu_class_ips:
      t4: ["10.20.1.70", "10.20.1.71"]
      l4: ["10.20.1.72"]
      a100: ["10.20.1.73"]
    """
    with open(ip_yaml_path, "r", encoding="utf-8") as f:
        cfg = yaml.safe_load(f)
    dispatcher_ip = cfg.get("dispatcher_ip")
    class_to_ips = cfg.get("gpu_class_ips") or {}
    if dispatcher_ip is None:
        raise ValueError("dispatcher_ip is required in the IP YAML")
    return dispatcher_ip, class_to_ips


def build_cluster_ips_for_plans(json_files, class_to_ips: dict) -> list:
    """Derive cluster IP list indexed by machine_id from planning JSONs and class IP pools.

    We scan all JSONs to map machine_id -> gpu_class (from stages[*].gpu_allocation and stages[*].gpu_class),
    then assign IPs to machine_ids by pulling from class_to_ips in order.
    """
    machine_to_class: dict[int, str] = {}
    max_machine_id = -1
    for json_file in json_files:
        ...
        stages = data.get(
            "stages",
            (
                data.get("pipeline_stages", {}).get("stages")
                if isinstance(data.get("pipeline_stages"), dict)
                else data.get("pipeline_stages")
            ),
        )
        if not stages:
            continue
        for st in stages:
            st_class = str(st.get("gpu_class", "unknown"))
            gpu_alloc = st.get("gpu_allocation", {})
            for mid_str in gpu_alloc.keys():
                try:
                    mid = int(mid_str)
                except Exception:
                    continue
                machine_to_class[mid] = st_class
    ...
    # Prepare counters per class
    class_to_index = defaultdict(int)
    cluster_ips = [None] * (max_machine_id + 1)
    for mid in sorted(machine_to_class.keys()):
        klass = machine_to_class[mid]
        ip_pool = class_to_ips.get(klass)
        if not ip_pool:
            raise ValueError(f"No IPs available for GPU class '{klass}' in IP YAML")
        idx = class_to_index[klass]
        if idx >= len(ip_pool):
            raise ValueError(
                f"Insufficient IPs for class '{klass}': need at least {idx+1}, have {len(ip_pool)}"
            )
        cluster_ips[mid] = ip_pool[idx]
        class_to_index[klass] += 1

    # Ensure no Nones remain; fill with any available IP (should not happen if YAML matches plan), else error
    for i, ip in enumerate(cluster_ips):
        if ip is None:
            raise ValueError(
                f"Missing IP assignment for machine {i}; ensure planning JSON uses it or provide mapping"
            )
    return cluster_ips


def process_multiple_json_configs(
    json_files,
    model_type,
    max_inflight,
    dispatcher_device,
    ...
    # Load dispatcher and class IP mapping
    dispatcher_ip, class_to_ips = load_ip_config(ip_yaml_path)
    # Build cluster IPs by mapping machine_id -> class from the planning JSONs
    cluster_ips = build_cluster_ips_for_plans(json_files, class_to_ips)
    ...
    for json_file in json_files:
        ...
        for worker in result["workers"]:
            if worker["id"] != "s-0":
                ...
                worker_id = worker["id"]
                for pipeline_worker_id, machine_id in result.get(
                    "worker_to_machine", {}
                ).items():
    ...
    # Now add the server/dispatcher once for all pipelines on a dedicated machine (from ip.yaml)
    # Enforce GPU placement for dispatcher on its own dedicated node.
    server_worker = {
        "id": "s-0",
        "device": "cuda:0",
        "is_server": True,
    ...
    config = {
        ...
        "nfaults": 1,
        "micro_batch_size": json_data[
            "batch_size"
        ],  # Use batch size from the last JSON
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="YAML path with dispatcher_ip and gpu_class_ips",
    )
    ...
    if len(args.input_jsons) > 1:
        output_path = (
            Path("infscale_config")
            / f"multi_{len(args.input_jsons)}_pipelines_cuda_dispatcher.yaml"
        )
    else:
        output_path = Path("infscale_config") / input_path.name.replace(
            ".json", "_cuda_dispatcher.yaml"
        )
