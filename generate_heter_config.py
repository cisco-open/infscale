...
def build_cluster_ips_for_plans(json_files, class_to_ips: dict) -> list:
    ...
    for json_file in json_files:
        with open(json_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
        stages = data.get('stages', data.get('pipeline_stages', {}).get('stages') if isinstance(data.get('pipeline_stages'), dict) else data.get('pipeline_stages'))
        if not stages:
            continue
        for st in stages:
    ...
    for mid in sorted(machine_to_class.keys()):
        ...
        if idx >= len(ip_pool):
            raise ValueError(f"Insufficient IPs for class '{klass}': need at least {idx+1}, have {len(ip_pool)}")
    ...
    for i, ip in enumerate(cluster_ips):
        if ip is None:
            raise ValueError(f"Missing IP assignment for machine {i}; ensure planning JSON uses it or provide mapping")
    return cluster_ips


def process_multiple_json_configs(json_files, model_type, max_inflight, dispatcher_device, gpus_per_machine, ip_yaml_path: str):
...
def json_to_yaml_config(json_files, output_path, model_type="resnet", max_inflight=None, dispatcher_device="cpu", gpus_per_machine=4, ip_yaml_path: str = ""):
...
def main():
    ...
    parser.add_argument('--ip_yaml', type=str, required=True,
