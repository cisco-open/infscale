...
def dp_pipeline_packing(gpus_per_node, num_nodes, num_templates, throughput, need_dispatcher: bool = False):
...
def dp_pipeline_packing_fault_tolerant(gpus_per_node: int,
                                       num_nodes: int,
                                       num_templates: int,
                                       throughput: list,
                                       k: int,
    ...
    DP   = [[[NEG]*(D_CAP+1) for _ in range(gpus_per_node)] for _ in range(num_nodes+1)]
    prev = [[[None]*(D_CAP+1) for _ in range(gpus_per_node)] for _ in range(num_nodes+1)]
    DP[0][0][0] = 0
            ...
            for d in range(D_CAP+1):
                ...
                for i in range(1, num_templates + 1):
                    ...
                    if c > 0 and r_i > 0 and c + r_i <= gpus_per_node:
                        ...
                        if n_new <= num_nodes and base + throughput[i] > DP[n_new][c_new][d]:
                    ...
                    if c == 0:
                        ...
                        if n_new <= num_nodes and base + throughput[i] > DP[n_new][c_new][d_new]:
...
def assign_nodes_with_deployments(solution, gpus_per_node):
    ...
    for t in solution:
        ...
        for _ in range(w_i):
            ...
            deployments[deploy_id]["node_segments"].append({"node_id": node_id, "gpus": gpus_per_node})
        ...
        if r_i > 0:
            ...
            deployments[deploy_id]["node_segments"].append({"node_id": node_id, "gpus": r_i})
...
if __name__ == "__main__":
    ...
    parser.add_argument('-k', '--fault_tolerance', type=int, default=0,
                        help='Number of node failures (k) to tolerate. 0 = no fault tolerance (default).')
    parser.add_argument('-o', '--out', type=str, default='placement.json',
    ...
    gpu_throughputs = {
        int(k): v[0]['throughput']
        for k, v in solutions_data.items() if v
    }
    ...
    nodes_dict, deployments_dict = assign_nodes_with_deployments(solution, gpus_per_node)
    ...
    deployed_sizes = {int(s.split('-')[0]) for s in deployments_dict.keys() if s != 'dispatcher'}
    template_solutions = {}
    for sz in deployed_sizes:
        entry = solutions_data.get(sz) if sz in solutions_data else solutions_data.get(str(sz))
        if isinstance(entry, list) and entry:
            tpl_filename = entry[0].get("template_name") if isinstance(entry[0], dict) else None
            if tpl_filename:
